...
int abs_compare(std::string firstNum, std::string secondNum) {
	if (firstNum.size() != secondNum.size()) { return firstNum.size() < secondNum.size() ? -1 : 1; }

	for (int i = 0; i < ((int) firstNum.size()); i++) 
		if (firstNum[i] != secondNum[i]) { return firstNum[i] < secondNum[i] ? -1 : 1; }
